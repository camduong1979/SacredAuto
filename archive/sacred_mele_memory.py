...
class SacredBotMemory:
    ...
    def print_hud(self, hp, threat, monster_count=0, hover_id=0, exp=0):
        ...
        [UPDATE 2026-08-30] Rút gọn HUD: chỉ giữ HP | Target | Event (bỏ Thr, Mobs, Hover, EXP).
        """
        now = time.time()
        ...
        hud_line = (
            f"\rHP: {hp:5.1f}% | Target: {target_str:<12} | {event_short}\033[K"
        )
    ...
    def cast_buff_guarded(self, target_buff):
        """[NEW 2026-08-30] Thi triển Buff theo cơ chế Phân đoạn có Kiểm tra (Step-by-Step Guarded Execution):
        1. Bước 1 (Select): Gửi phím chọn skill (ví dụ phím '9').
        2. Pre-Cast Check: Đọc Memory Cooldown Hook (hoặc Sentinel).
           - Nếu Cooldown > 0.001 (Skill đang hồi chiêu / chưa sẵn sàng):
             -> Gửi ngay phím phục hồi ('6') để trả về phím chính.
             -> Trả về False (để kích hoạt retry/hoãn chu kỳ, không bấm chuột lãng phí).
           - Nếu Cooldown <= 0.001 (Skill sẵn sàng 100%): Đi tiếp sang Bước 2.
        3. Bước 2 (Cast): Click chuột phải thi triển ('click_right').
        4. Post-Cast Verification: Đọc lại Cooldown Hook (hoặc Sentinel).
           - Nếu Cooldown > 0.001 (hoặc Sentinel tối): Buff thành công 100%.
             -> Gửi phím phục hồi ('6') trả về phím chính.
             -> Trả về True.
           - Nếu Cooldown <= 0.001: Buff bị hụt (miss click/lag).
             -> Gửi phím phục hồi ('6') trả về phím chính.
             -> Trả về False (để kích hoạt Fast-Retry).
        """
        ...
        time.sleep(0.05)  # Chờ game cập nhật memory pointer / UI state

        # BƯỚC 1.5: Pre-Cast Check (Kiểm tra xem skill có sẵn sàng = 0 không)
        is_ready = True
        if mode == 'memory' and self.cooldown_hook and self.cooldown_hook.is_hooked:
            cd_val = self.cooldown_hook.get_cooldown()
        ...
        if not is_ready:
            # Skill đang hồi chiêu / đang cast -> Trả về phím chính ngay, không click chuột
        ...
        # --- OLD CODE (REPLACED: đọc cooldown 1 lần duy nhất sau 100ms cứng → false negative khi game lag) ---
        # time.sleep(0.10)  # Chờ game nhận diện cast & cập nhật cooldown
        # cast_success = False
        # if mode == 'memory' and self.cooldown_hook and self.cooldown_hook.is_hooked:
        #     cd_after = self.cooldown_hook.get_cooldown()
        ...
        # BƯỚC 2.5: Post-Cast Verification — Poll tối đa 3 lần, cách nhau 60ms (~180ms tổng)
        # Dừng sớm ngay khi xác nhận thành công → không chờ lãng phí nếu game phản hồi nhanh
        cast_success = False
        for _ in range(3):
            time.sleep(0.06)  # Chờ 60ms mỗi vòng để game kịp cập nhật cooldown / sentinel
            if mode == 'memory' and self.cooldown_hook and self.cooldown_hook.is_hooked:
                cd_after = self.cooldown_hook.get_cooldown()
                if cd_after is not None and cd_after > 0.001:
                    cast_success = True
                    break  # Xác nhận thành công → thoát sớm, không poll thêm
            else:
                # Chế độ visual hoặc fallback sentinel
                ...
                if self.buff_queue['enabled']:
                    ...
                    if current_time - self.last_buff_finish_time >= self.last_buff_cast_delay:
                        ...
                        if target_buff:
                            ...
                            # --- OLD CODE (REPLACED: Gửi batch sequence cũ rồi mới verify) ---
                            ...
                            # [NEW 2026-08-30] THI TRIỂN THEO BƯỚC CÓ XÁC THỰC (Step-by-Step Guarded Execution)
                            ...
                            if cast_success:
                                ...
                                # Bị miss hoặc skill chưa sẵn sàng -> Kích hoạt Fast-Retry
    ...
    def memory_target_worker(self):
        ...
        [UPDATE 2026-08-30] Xác nhận đa tầng: Hover ID → monster_ids → HP Bar màu (CombatRadar).
        Tầng 1: hover_id > 1          → loại nhân vật (=1) và đất (=0)
        Tầng 2: hover_id in monster_ids → match RAM quái sống
        Tầng 3: radar.is_target_detected() → HP bar màu còn hiển thị (quái chưa chết)
        """
        while not self.exit_event.is_set():
            ...
            try:
                ...
                # --- OLD CODE (REPLACED: 2 tầng cũ, thiếu Tầng 3 HP color) ---
                ...
                # TẦNG 1: hover_id > 1 — loại đất (0) và nhân vật bản thân (1)
                if hover_id > 1:
                    # TẦNG 2: đối khớp ID với danh sách quái sống trong RAM
                    is_id_match = hover_id in monster_ids

                    # TẦNG 3: xác nhận HP bar màu còn hiển thị (quái chưa chết)
                    is_hp_visible = self.radar.is_target_detected() if self.radar else False

                    if is_id_match and is_hp_visible:
                        # Cả 3 tầng đều khớp → khóa mục tiêu
                        self.target_detected = True
                        self.target_locked_id = hover_id
                        self.last_event_msg = f"Target Lock: {hover_id}"
                    elif is_id_match and not is_hp_visible:
                        # ID khớp nhưng HP bar biến mất → quái chết lan → nhả lock ngay
                        self.target_detected = False
                        self.target_locked_id = 0
                        self.last_event_msg = "Target: HP gone (dead)"
                    else:
                        # ID không khớp (NPC / đối tượng khác) → nhả lock
                        self.target_detected = False
                        self.target_locked_id = 0
                else:
                    # hover_id <= 1: trỏ vào đất hoặc nhân vật → luôn nhả lock
                    # Kiểm tra thêm: nếu quái đang locked bị xóa khỏi monster_ids → cũng nhả
                    if self.target_locked_id > 0 and self.target_locked_id not in monster_ids:
                        self.target_detected = False
                        self.target_locked_id = 0
                    elif self.target_locked_id > 0:
                        # Quái vẫn trong RAM nhưng chuột đã rời → giữ lock, kiểm tra HP bar
                        is_hp_still = self.radar.is_target_detected() if self.radar else False
                        if not is_hp_still:
                            self.target_detected = False
                            self.target_locked_id = 0
                            self.last_event_msg = "Target: HP gone (dead)"
