...
from SkillCooldownClass import SkillCooldownManager  # [NEW] Memory Cooldown Hook
...
class SacredBot:
    def __init__(self):
        ...
        self.cooldown_hook = None  # [NEW] Bộ Hook Memory Cooldown Skill
        ...
        # [NEW] Trạng thái thực tế chuột (Manual Mouse Arbiter)
        self.is_left_down = False       # Phím Z -> chuột trái
        self.is_right_down = False      # Phím X -> chuột phải
    ...
    # --- OLD CODE (REPLACED: print_hud cũ hiển thị cả Threat, không có status ON/OFF) ---
    ...
    def print_hud(self, hp):
        """[UPDATE] In trạng thái trực tiếp trên 1 dòng duy nhất (Single-line Live HUD).
        Thu gọn: Bot status + HP + Event ngắn. Debug buff xuất ra print() riêng, không ghi đè HUD.
        """
        now = time.time()
        ...
        # Triệt tiêu toàn bộ ký tự xuống dòng trong msg để không làm vỡ HUD
        clean_msg = (
        ...
        status_text = "ON " if self.is_running else "OFF"
        hud_line = (
            f"\r[{status_text}] HP: {hp:5.1f}% | {event_short}\033[K"
        )
    ...
    # --- OLD CODE (REPLACED: _init_buff_system v2.0 chỉ Visual Sentinel, thiếu mode/fast_retry/retry_count) ---
    ...
    #     return {'enabled': enabled, 'buffs': buffs}
    # ---------------------------------------------------------------------------------------------------------

    def _init_buff_system(self):
        """[NEW] Khởi tạo hệ thống Auto Buff v3.0 từ config.
        Dual-mode: Visual Sentinel (Combo) + Memory Cooldown Hook (Magic/CA).
        Post-Cast Verification + Smart Fast-Retry độc lập cho từng buff.
        """
        buff_cfg = self.config.get('auto_buff_system', {})
        buffs = []
        for buff_data in buff_cfg.get('buffs', []):
        ...
        enabled = buff_cfg.get('enabled', False)
        print(f"[BUFF SYSTEM v3.0] {'BẬT' if enabled else 'TẮT'} — Loaded {len(buffs)} loại buff (Dual-Mode Scheduler Active).")
        return {
            'enabled': enabled,
            'buffs': buffs
        }

    # --- OLD CODE (REPLACED: is_buff_ready cũ chỉ dùng Visual Sentinel đơn giản) ---
    ...
    def is_buff_ready(self, buff):
        """[NEW] Kiểm tra trạng thái sẵn sàng của Buff theo từng chế độ độc lập.
        1. Chế độ Memory (Magic, CA): Đọc trực tiếp Cooldown từ Memory Hook.
    ...
    # --- OLD CODE (REPLACED: cast_buff_guarded cũ chỉ poll 3x60ms=180ms quá ngắn gây false-positive Miss khi nhân vật đang animation) ---
    ...
    #             return cd_after is not None and cd_after > 0.001
    #         return not self.is_buff_ready(target_buff)
    ...
    #     if mode == 'memory' and self.cooldown_hook and self.cooldown_hook.is_hooked:
    #         cd_val = self.cooldown_hook.get_cooldown()
    #         if cd_val is not None and cd_val > 0.001:
    #             is_ready = False
    #     elif mode == 'visual' and target_buff.get('sentinel_enabled', True):
    #         is_ready = self.is_buff_ready(target_buff)
    #     if not is_ready:
    ...
    def cast_buff_guarded(self, target_buff):
        """[UPDATE] Thi triển Buff theo cơ chế Phân đoạn có Kiểm tra (Step-by-Step Guarded Execution):
        1. Bước 1 (Select)   : Gửi phím chọn skill (ví dụ phím 7 hoặc 9).
        2. Pre-Cast Check    : Đọc Memory Cooldown Hook (hoặc Sentinel).
           - Cooldown > 0.001 -> Chưa sẵn sàng -> Trả về phím chính ngay, không click chuột -> False.
           - Cooldown <= 0.001-> Sẵn sàng -> Đi tiếp sang Bước 2.
        3. Bước 2 (Cast)     : Click chuột phải thi triển skill.
        4. Post-Cast Verify  : Mở rộng cửa sổ poll tối đa 7 lần × 60ms (~420ms) kết hợp kiểm tra animation delay.
           - Dừng sớm ngay khi Cooldown > 0.001 (hoặc Sentinel tối) -> Xác nhận Thành công 100%.
           - Sau khi cast xong, chờ thêm một nhịp animation an toàn trước khi bấm phím 6 để tránh ngắt skill.
        5. Bước 3 (Restore)  : Trả về phím chính (phím 6).
        """
        sequence = target_buff.get('sequence', [])
        if not sequence:
            return False

        # Nếu sequence không đủ 3 bước chuẩn [select, cast, restore], fallback chạy toàn bộ chuỗi
        if len(sequence) < 3:
            self.hotkey_sys.execute_sequence(sequence)
            time.sleep(0.15)
            mode = target_buff.get('mode', 'memory' if target_buff.get('use_memory') else 'visual')
        ...
        # Clean state trước khi cast
        self.hotkey_sys.release_all_inputs()
        time.sleep(0.01)

        # BƯỚC 1: Chọn phím skill buff
        self.hotkey_sys.execute_step(select_step)
        select_wait = select_step.get('wait', 0.05)
        time.sleep(max(0.05, select_wait))  # Chờ game cập nhật memory pointer / UI state

        # BƯỚC 1.5: Pre-Cast Check (Kiểm tra xem skill có sẵn sàng không)
        is_ready = True
        if mode == 'memory' and self.cooldown_hook and self.cooldown_hook.is_hooked:
            cd_val = self.cooldown_hook.get_cooldown()
            if cd_val is not None and cd_val > 0.001:
        ...
        if not is_ready:
            # Skill đang hồi chiêu -> Trả về phím chính ngay, không click chuột
            for step in restore_steps:
                self.hotkey_sys.execute_step(step)
            return False

        # BƯỚC 2: Thi triển skill (Click chuột)
        self.hotkey_sys.execute_step(cast_step)

        # BƯỚC 2.5: Post-Cast Verification — Mở rộng poll tối đa 7 lần × 60ms (~420ms)
        # Giúp bao phủ trọn vẹn thời gian animation cast của game mà không bị miss
        cast_success = False
        for _ in range(7):
            time.sleep(0.06)
            if mode == 'memory' and self.cooldown_hook and self.cooldown_hook.is_hooked:
                ...
                if cd_after is not None and cd_after > 0.001:
                    cast_success = True
                    break  # Đã kích hoạt cooldown thành công -> dừng poll sớm
            else:
                if not self.is_buff_ready(target_buff):
                    cast_success = True
                    break  # Sentinel tối -> skill đang cooldown -> cast thành công

        # Chờ phần animation còn lại của cast_step (nếu có) trước khi switch phím để tránh hủy chiêu
        cast_wait = cast_step.get('wait', 0.2)
        if cast_wait > 0.3:
            time.sleep(min(0.2, cast_wait - 0.2))

        # BƯỚC 3: Trả về phím chính
        for step in restore_steps:
            self.hotkey_sys.execute_step(step)

        return cast_success

    # --- OLD CODE (REPLACED: connect_game cũ chưa khởi tạo cooldown_hook) ---
    ...
    def connect_game(self):
        ...
        try:
            ...
            # [NEW] Khởi tạo & Cài đặt Cooldown Memory Hook
    ...
    # --- OLD CODE (REPLACED: action_worker cũ tuần tự, thiếu Guarded Cast / Fast-Retry / Debug logging) ---
    ...
    def action_worker(self):
        """[UPDATE] Luồng thực thi: Auto Buff v3.0 (Dual-Mode Decoupled Scheduler + Guarded Cast) + Bơm máu.
        Bỏ hoàn toàn phần tự động đánh quái. Debug buff in ra print() riêng biệt.
        """
        last_potion_time = 0
        ...
        while not self.exit_event.is_set():
            ...
            if threat > 0:
                ...
                # THÔNG BÁO GIỌNG NÓI (Chỉ nói 1 lần duy nhất khi vừa chớm gặp bãi quái)
                if not self.is_in_combat:
                    self.is_in_combat = True
                    self.voice.speak("Có quái.")

                # [NEW] HỆ THỐNG BUFF ĐỘC LẬP (Decoupled Scheduler + Post-Cast Verification + Smart Fast-Retry)
                if self.buff_queue['enabled']:
                    # Kiểm tra Kênh Thi Triển Không Nghẽn (Cast Channel Free)
                    if current_time - self.last_buff_finish_time >= self.last_buff_cast_delay:
                        target_buff = None

                        # Ưu tiên 1: Các buff đang trong trạng thái Fast-Retry cần thử lại khi đến hạn
                        ...
                        if target_buff:
                            ...
                            retry_label = f" (Retry #{target_buff['retry_count']})" if is_retry else ""

                            # --- DEBUG: Thông báo bắt đầu cast (xuống dòng riêng, không ghi đè HUD) ---
                            print(f"\n[BUFF] ▶ Casting{retry_label}: {buff_name}")

                            self.last_event_msg = f"Buff: {buff_name}" + retry_label
                            if not is_retry:
                                self.voice.speak(buff_name)

                            # THI TRIỂN THEO BƯỚC CÓ XÁC THỰC (Guarded Execution)
                            cast_success = self.cast_buff_guarded(target_buff)

                            # PHÂN NHÁNH XỬ LÝ KẾT QUẢ (Success / Fast-Retry / Missed)
                            now_time = time.time()
                            if cast_success:
                                # Thi triển thành công -> Reset retry và cập nhật chu kỳ mới
                                ...
                                # --- DEBUG: Thành công ---
                                print(f"[BUFF] ✓ OK: {buff_name}")
                            else:
                                # Miss hoặc skill chưa sẵn sàng -> Kích hoạt Fast-Retry
                                ...
                                if target_buff['retry_count'] <= max_retries:
                                    ...
                                    # --- DEBUG: Miss, kích hoạt retry ---
                                    print(f"[BUFF] ✗ Miss: {buff_name} → Retry #{target_buff['retry_count']} sau {retry_delay:.1f}s")
                                else:
                                    # Quá số lần retry -> Hủy lượt, chuyển sang chu kỳ tiếp theo
                                    target_buff['last_cast_time'] = now_time
                                    target_buff['retry_count']    = 0
                                    target_buff['next_retry_time'] = 0.0
                                    self.last_event_msg = f"Buff {buff_name}: Missed"
                                    # --- DEBUG: Hết retry, bỏ lượt ---
                                    print(f"[BUFF] ✗ Missed (hết retry): {buff_name}")

                            self.last_buff_finish_time = time.time()
                            self.last_buff_cast_delay  = target_buff['cast_delay']

            else:
                # XỬ LÝ KHI QUÉT SẠCH QUÁI (Safe Timer dùng giây thực)
                if self.is_in_combat:
                    if self.safe_start_time == 0.0:
                        self.safe_start_time = current_time
                    elif current_time - self.safe_start_time > 3.0:
    ...
    def mouse_arbiter_worker(self):
        """[NEW] Bộ điều khiển chuột thủ công (Manual-only, không auto combat).
        - Phím Z: Đè -> mouseDown('left'),  Nhả -> mouseUp('left')  để di chuyển / đánh thường.
        - Phím X: Đè -> mouseDown('right'), Nhả -> mouseUp('right') để dùng chiêu chuột phải.
        """
        while not self.exit_event.is_set():
            if not (self.game_connected and self.is_running):
            ...
            # CHUỘT TRÁI (PHÍM Z)
            if z_down and not self.is_left_down:
                pydirectinput.mouseDown(button='left')
                self.is_left_down = True
            elif not z_down and self.is_left_down:
                pydirectinput.mouseUp(button='left')
                self.is_left_down = False
    ...
    # --- OLD CODE (REPLACED: run cũ thiếu mouse_arbiter_worker, thiếu try/finally cho cooldown_hook) ---
    # def run(self):
    #     threads = [
        ...
        try:
            while not self.exit_event.is_set():
                ...
                if keyboard.is_pressed(toggle_key):
                    ...
                    winsound.Beep(1000 if self.is_running else 500, 200)
                    if self.is_running:
                        # Reset tất cả buff timers khi bật lại
                        for buff in self.buff_queue['buffs']:
            ...
            # Đảm bảo uninstall hook an toàn khi thoát dù có exception hay không
